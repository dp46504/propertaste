  ...
  const wyswietlanieWartosci = (czasWMs, temperaturaInfo) => {
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        document.querySelector("#ciekawostkiBox").classList +=
          " animacjaPojawianie";
        document.querySelector("#ciekawostkiBox").innerHTML =
          resObj["ciekawostka1"];
        setTimeout(() => {
          document.querySelector("#ciekawostkiBox").classList -=
            " animacjaPojawianie";
    ...
    children[0].addEventListener("click", function handler() {
      ...
      // Dispatch eventu do google analytics
      logEvent(analytics, "start_timer", {
        typ_herbaty: herbata,
      });
